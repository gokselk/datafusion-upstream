...
mod tests {
    ...
    async fn join_inner_one_two_parts_left(batch_size: usize) -> Result<()> {
        ...
        let left = Arc::new(
            MemoryExec::try_new(&[vec![batch1], vec![batch2]], schema, None, None).unwrap(),
        );
    ...
    fn build_table_two_batches(
        ...
        let schema = batch.schema();
        Arc::new(
            MemoryExec::try_new(&[vec![batch.clone(), batch]], schema, None, None).unwrap(),
        )
    }
    ...
    async fn join_left_empty_right(batch_size: usize) {
        ...
        let right = Arc::new(MemoryExec::try_new(&[vec![right]], schema, None, None).unwrap());
        let join = join(left, right, on, &JoinType::Left, false).unwrap();
    ...
    async fn join_date32() -> Result<()> {
        ...
        let batch = RecordBatch::try_new(Arc::clone(&schema), vec![dates, n])?;
        let right = Arc::new(MemoryExec::try_new(&[vec![batch]], schema, None, None).unwrap());
